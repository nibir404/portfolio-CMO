...
async function getArticle(slug: string) {
  try {
    ...
    const article = await db
      .collection<ArticleData>(COLLECTIONS.ARTICLES)
      .findOne({ slug });
...
export default async function ArticlePage({ params }: Props) {
  ...
  const profile = await getSingleton<{ name: string; jobTitle: string }>(
    COLLECTIONS.PROFILE,
  );
    ...
    <>
      ...
      <main
        style={{
          ...
          paddingBottom: "120px",
          minHeight: "100vh",
          background: article.styling?.bgColor || "var(--color-bg)",
        }}
      >
        <Container>
          <div style={{ maxWidth: "800px", margin: "0 auto" }}>
            ...
            <div style={{ marginBottom: "2rem" }}>
              <Link
                href="/news"
                style={{
                  color: article.styling?.accentColor || "var(--color-accent)",
                  textDecoration: "none",
                  fontSize: "0.9rem",
                  fontWeight: 600,
                  display: "inline-flex",
              ...
              {article.category && (
                <div style={{ marginBottom: "1rem" }}>
                  <span
                    style={{
                      color:
                        article.styling?.accentColor || "var(--color-accent)",
                      fontWeight: 600,
            ...
            {article.coverImage &&
              (!article.styling?.coverStyle ||
                article.styling?.coverStyle === "standard") && (
                <div
                  style={{
                    marginBottom: "3rem",
                    borderRadius: "12px",
